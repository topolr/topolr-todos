/*!
 * TopolrJS JavaScript Library v0.12.2
 * http://topolr.org/
 * Author WangJinliang(hou80houzhu)
 * licensed under the MIT licenses(https://github.com/topolr/topolrjs/blob/master/LICENSE).
 */
(function () {
    "use strict";
            ...
            if (!loader.importsmapping.hasJs(jspath)) {
                if (async) {
                    ...
                    topolr.ajax({
                        ...
                        async: false,
                        success: function (e) {
                            (new Function("try{" + e + "}catch(e){console.error('[topolr]imports: %s ,path " + jspath + "',e.stack);}"))();
                            if (callback) {
                                callback();
                            }
                        },
            ...
            if (baseMapping.update === 0 || baseMapping.currentBuild <= baseMapping.update || baseMapping.currentBuild > cid) {
                if (baseMapping.currentBuild !== 0) {
                    ...
                    try {
                        ...
                        console.error("[topolr] onappupdate called error " + e.stack);
                    }
                }
            }
        } else {
        ...
        topolr().ready(function () {
            ...
            packet.preload(function () {
                packet.pretreat().done(function () {
                    ...
                    baseMapping.debug && console.log("[topolr] version:0.10.4,debug:" + baseMapping.debug + ",basePath:" + baseMapping.basePath);
                    var root = topolr("*[data-view='root']");
                    if (root.length > 0) {
                        module.getViewInstance(root, optionName, function (a) {
                            a.privator("render");
                        });
                    } else {
                        throw Error("[topolr boot] can not find the root element.");
                    }
                });
            });
        });
    };
    topolr.App = function (obj) {
...
//                ops.packetName = ops.packetName.replace(packet.issuffix, "");
                        ...
                        for (var i = 0; i < ths.info.length; i++) {
                            ...
                            try {
                                ...
                                new Function("$", "Module", "Option", "module", "exports", "require", "Plugin", "Method", xcode).call(
                                        d, topolr, topolr.Module, topolr.Option, d, {},
                                        function (packetName) {
        ...
        return code.replace(/src=['"].+?['"]/g, function (a) {
            ...
            if (a.indexOf("<%") === -1) {
                ...
                if(newpath.indexOf("http:")===0||newpath.indexOf("data:")===0||newpath.indexOf("https:")===0){
                    ...
                    if (!done) {
                        rp = "src=\"" + baseMapping.basePath + newpath + "\"";
                    }
                    return rp;
                }
            } else {
    ...
    topolr.packet = function (option) {
        new packet(option);
    };
    topolr.source = function (a) {
        for (var type in a) {
            var fileload = true;
        ...
        for (var q = 0; q < keys.length; q++) {
            ...
            (function (t) {
                obj.__defineSetter__(t, function (a) {
                    try {
                        this["_*_"].fn && this["_*_"].fn(topolr.extend(new command(), {
                            name: this["_*_"].name,
                            dataType: "object",
                            type: "edit",
                            object: this,
    ...
    observe.setObserve = function (name, key, obj, parent, fn) {
        ...
        } else if (topolr.is.isObject(obj)) {
            return observe.setObject(name, key, obj, parent, fn);
        } else {
            return obj;
        }
    };
    topolr.observe = function (name, obj, fn) {
    ...
    node.prototype.element = function () {
        var t = topolr().create(this.tag).attr(this.props).get(0);
        for (var i = 0; i < this.children.length; i++) {
...
//        console.log(t);
                        ...
                        back: function () {
                            if (fn) {
                                if (module.has(moduleName)) {
                                    ...
                                    throw Error("[topolr] can not find module with name of " + moduleName + ",it is not in the packet of " + module.getbrightName(moduleName) + " or the packet file inited failed.");
                                }
                            }
                        }
                    });
                            ...
                            back: function () {
                                if (fn) {
                                    ...
                                    if (ops) {
                                        ...
                                        throw Error("[topolr] can not find option with name of " + optionName + ",is not in the packet of " + module.getbrightName(optionName));
                                    }
                                }
                            }
                        });
    ...
    delegater.group = function (module) {
        ...
        module.dom.find("*[data-group]").each(function () {
            ...
            topolr(this).find("*[data-groupi]").each(function () {
                ...
                topolr(this).data("-groupitem-", {name: _name, group: qt}).removeAttr("data-groupi").attr("groupi", _name);
            });
    ...
    module.add({
        ...
        getUUID: function () {
            return topolr.util.uuid();
        },
        observe: function (name, obj) {
            var ths = this;
            return topolr.observe(name, obj, function (e) {
        ...
        getStaticJson: function (packet) {
            var ps = topolr.promise().scope(this);
    ...
    module.add({
        ...
        _addChild: function (option, callback) {
            var ths = this, ops = topolr.extend({type: null,
                option: "",
                parameters: null,
    ...
    topolr.Plugin = function (obj) {
        if (obj && obj.name && obj.name !== "") {
            ...
            topolr.fn[obj.name] = function (option) {
    ...
    topolr.Method = function (obj) {
        if (obj && obj.name && obj.name !== "") {
            topolr[obj.name] = function (option) {
    ...
    topolr.overrideRoot = function (obj) {
